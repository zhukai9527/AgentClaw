...
### 新功能
- **文档知识库（RAG）**：agent 可上传文档文件（.txt/.md/.csv 等），系统自动切片 → embedding → 向量索引；LLM 通过自动生成的搜索工具按语义检索相关段落；支持 500 字分块、段落/句子边界对齐、100 字重叠；知识源类型简化为两种：HTTP API + 静态文件（MCP 通过现有工具白名单机制使用）
